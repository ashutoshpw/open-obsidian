...
| `bun run audit:electron-vault` | Launches the local Electron shell against a disposable existing vault, verifies mediated open/read/write and a fresh-process reopen; the quality matrix runs it on Ubuntu, macOS and Windows while reference Obsidian and human validation remain pending. |
