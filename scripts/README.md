...
| `bun run audit:plugin-artifact-lifecycle` | Runs every pinned `main.js` through the hidden Electron deny-capability renderer lifecycle wrapper after integrity verification; this is artifact-specific module/lifecycle evidence only and never grants production trust. |
| `bun run validate:plugin-d15` | Builds per-workflow D15 candidate records from the pinned artifact lifecycle evidence, checks visible pending compatibility entries and executes synthetic denied-capability paths; it never promotes a plugin to supported or unsupported-security. |
| `bun run audit:plugin-lifecycle` | Runs synthetic marker-free onload/onunload, restart/update/uninstall, command/view/settings/persistence and return-to-Obsidian boundary traces plus a synthetic privileged-DOM denial in the Electron renderer wrapper; unchanged plugin lifecycle, cross-platform enforcement and reference behavior remain pending. |
| `bun run audit:plugin-loaded-workflows` | Runs unchanged pinned loaded-workflow traces for the selected artifacts and required combinations inside the hidden Electron sandbox; combination probes disable automatic writers by default and record deterministic persisted-state deny/clear/return recovery, while action, persistence, cross-platform and reference behavior remain pending. |
| `bun run audit:vault-roundtrip` | Runs a bounded real temporary-directory scan, byte-preserving edit, recovery, reopen, separator, symlink and reserved-name report on the host platform; reference-vault and fault-environment gates remain pending. |
| `bun run audit:electron-vault` | Launches the local Electron shell against disposable existing vaults, verifies `--vault/--open` renderer hydration, keyboard-save/Unicode/mode-switch behavior, mediated open/read/write and a fresh-process reopen; the quality matrix runs it on Ubuntu, macOS and Windows while reference Obsidian and human validation remain pending. |
