...
| `bun run audit:plugin-loaded-workflows` | Runs unchanged pinned loaded-workflow traces for the selected artifacts and their combination inside the hidden Electron sandbox; the same audit runs on the Ubuntu/macOS/Windows matrix and keeps action, persistence, cross-platform and reference behavior pending. |
