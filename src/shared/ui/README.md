...
- Markdown properties keep their raw source spans while the core read path exposes a bounded YAML mapping for nested arrays/maps. `serializeYamlValue` and typed property edits use representable scalar/flow values; `editMarkdownNestedPropertyValue` can update an existing inline leaf under block-style mappings, flow mappings, an explicitly indexed block sequence map or an explicitly indexed flow sequence without touching siblings or comments. Ambiguous sequence entries, malformed flow collections, block scalars and other unsupported YAML remain source-only and are refused rather than silently rewritten.
...
- `themes.ts` parses appearance settings and CSS into variables, legacy layout contracts, plugin-view/popout selectors and accessibility hints. It never executes raw CSS, resolves imports or fetches URL assets; desktop and Expo hosts can apply an explicit preview policy to the same analysis. The Electron renderer receives source and analysis through a read-only vault IPC method, scopes preview rules to the workspace shell and withholds privileged/document-level rules.
