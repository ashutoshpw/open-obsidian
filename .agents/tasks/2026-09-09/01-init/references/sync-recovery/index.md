...
Confirmed local history: 30 days and a configurable 5 GiB cap. Never automatically remove unresolved conflicts; warn when protected history exceeds the cap and require explicit resolution. Protect history with OS access controls and available full-disk encryption, not app-managed encryption. Do not claim full-disk encryption is enabled without checking. Uninstall must not delete the user's vault; cleanup offers explicit choices for app caches, credentials and history. Chronicle's Git history supplements this journal and does not authorize automatic pushes or destructive resets.
