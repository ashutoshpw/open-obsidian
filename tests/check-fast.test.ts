...
test("fast check covers routine quality gates without packaging", () => {
  ...
  expect(ids).toEqual(["state", "contracts", "plugin-matrix", "architecture", "surfaces", "version-baseline", "differential", "entry-points", "paid-service-boundary", "workflows", "accessibility", "layout", "typecheck", "tests", "knip", "fallow-changed"]);
