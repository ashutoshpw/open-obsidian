...
- `bun run validate:workflows` checks all 16 C10 workflows, their local test dispositions, renderer markers and external handoffs. The local bookmark index reads `.obsidian/bookmarks.json` without writing to the vault; tag and task indices scan Markdown, task checkbox writes require the current file revision, and templates/daily notes use bounded configuration-driven plain-text behavior. Popouts remain an explicit deferred handoff.
